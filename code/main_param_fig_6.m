...
%Example
load('/Volumes/GoogleDrive/My Drive/ODT/code/lippmann_paper/shepp_logan.mat',...
    'M', 'SP', 'f', 'n','simP', 'uM','uem')
%load('/Users/anpham/Google Drive/ODT/code/lippmann_paper/fine_grid_shep_noisy_n0_1.33_dn_0.13_Nviews_31_Nx_1024_GaussianBeam_0_noise_splitGauss_0_0_dB.mat',...
...
par.Lxext = simP.Lx;
par.Lzext = simP.Lz;% for simulated data
...
par.stochGrad = 2;%1: Stochastic Gradient descent or 2: switch of one with equally angle spaced Lsub angles
...
par.xtol = 1e-4;%Tolerance for stopping criterion
...
par.discreteGtild = true;%true for FFT based Gtild, false for matrix based Gtild
...
%the inner FISTA
par.Niterin = 120;%number of iterations for iterative forward/inner FISTA
par.Nupdatein = 0;%Nupdate > 0 activates displays every 50 iterations
par.xtolin = 1e-8;%Tolerance for stopping criterion
%and iterates saved in F.OutOpIn every Nupdate for SEAGLE

%TV ADMM parameters
par.penalty = 'l21';%'l20','l21'
par.regul = 3.3e-2;%Regularization parameter
par.bounds = [0,inf];%[0 inf];%set constraint (positivity [0 inf], or e.g. [0,0.1])
par.prox_iter = 2*7.5e2;%number of iterations for TV proximity operator
par.rhoTV = 4e1*[0.5,1];%Penalty parameters (rho) for ADMM (deprecated)
